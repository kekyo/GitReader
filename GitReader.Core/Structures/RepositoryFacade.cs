...
internal static class RepositoryFacade
{
    ...
    public static async Task<Commit[]> PrettyPrintAsync(
        Commit commit,
        TextWriter tw,
        Branch? head,
        CancellationToken ct)
    {
        var parentsTask = GetParentsAsync(commit, ct);
        ...
        tw.WriteLine($"Author: {Utilities.ToGitAuthorString(commit.Author)}");
        tw.WriteLine($"Date:   {Utilities.ToGitDateString(commit.Author.Date)}");
